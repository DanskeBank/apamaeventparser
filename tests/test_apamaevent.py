...
class TestApamaEvents(unittest.TestCase):
    ...
    def test_simple_fields(self):
        expected_event = ApamaEvent(package_name='heimdall.horn',event_name='ragnarok', fields=['"valhalla"', 1, 3.14, 1.0e6, False, True])
        tokens = tokenize('heimdall.horn.ragnarok("valhalla", 1, 3.14, 1.0e6, false, true)')
        parsed_event = event.parse(tokens)
        self.assertEqual(parsed_event, expected_event)

    def test_channel(self):
        expected_event = ApamaEvent(channel='channel',package_name='heimdall.horn',event_name='ragnarok', fields=['"valhalla"', 1, 3.14, 1.0e6, False, True])
...
class TestApamaEventParts(unittest.TestCase):
    ...
    def test_event_body_all_types(self):
        expected_result = ['"valhalla"', 1, 3.14, 1.0e6, True, False,[1,2,3,4],{'"Thor"': '"Mjolner"', '"Odin"': '"Gungnir"', '"Freja"': '"Falkedragt"'}]
        tokens = tokenize('("valhalla", 1, 3.14, 1.0e6, true, false,[1,2,3,4],{"Thor": "Mjolner", "Odin": "Gungnir", "Freja": "Falkedragt"})')
        parsed_package = event_body.parse(tokens)
        self.assertEqual(parsed_package, expected_result)
    ...
    def test_dictionary_in_dictionary(self):
        ...
        tokens = tokenize('{"other":{}, "Weapons": {"Thor": "Mjolner", "Odin": "Gungnir", "Freja": "Falkedragt"}, "Transportation": {"Odin": "Sleipner", "Thor": "Goats"}}')
        parsed_package = dictionary.parse(tokens)
        self.assertEqual(parsed_package, expected_result)
